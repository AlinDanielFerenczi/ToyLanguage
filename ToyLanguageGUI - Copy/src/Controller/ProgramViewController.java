...
import Model.ViewRepresentations.BarrierRepresentation;
...
public class ProgramViewController implements Initializable {
    ...
    public TableView<BarrierRepresentation> barrierTable;
    ...
    private void updateBarrierTable() {
        ObservableList<BarrierRepresentation> barrierTableContents = FXCollections.observableArrayList();
        IList<PrgState> prgList = programController.getPrgList();
        if(prgList.size() > 0)
            prgList.current(0)
                    .getBarrier()
                    .getContent()
                    .forEach((key, value) -> barrierTableContents.add(new BarrierRepresentation(key, value.getKey(), value.getValue())));
        barrierTable.setItems(barrierTableContents);
    }
